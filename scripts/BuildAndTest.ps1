...
if (-not $NoRestore) {
    Write-Information "Restoring NuGet packages for $SampleSolutionFile..."
        & dotnet nuget restore -ConfigFile $NuGetConfigFile -Verbosity $NuGetVerbosity -OutputDirectory $NuGetSamplesPackageRoot (Join-Path $SourceRoot $SampleSolutionFile)
    if ($LASTEXITCODE -ne 0) {
