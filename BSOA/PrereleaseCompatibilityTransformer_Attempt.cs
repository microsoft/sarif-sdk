    ...
    public static class PrereleaseCompatibilityTransformer
    {
        ...
        private static bool ConvertResultLogicalLocationToArray(JObject sarifLog)
        {
            ...
            if (sarifLog["runs"] is JArray runs)
            {
                foreach (JObject run in runs)
                {
                    modifiedLog |= PerformActionOnLeafNodeIfExists(
            ...
            if (sarifLog["inlineExternalProperties"] is JArray inlineExternalProperties)
            {
                foreach (JObject runProperty in inlineExternalProperties)
                {
                    modifiedLog |= PerformActionOnLeafNodeIfExists(
                        possiblePathsToLeafNode: resultPathsToUpdate,
                        rootNode: runProperty,
                        action: actionOnLeafNode);
                }
            }
        ...
        private static bool ApplyChangesFromTC35(JObject sarifLog)
        {
            ...
            modifiedLog |= RenameSuppressionKindValues(sarifLog);

            // https://github.com/oasis-tcs/sarif-spec/issues/375
            modifiedLog |= HoistIdsFromPhysicalLocationToLocation(sarifLog);

            modifiedLog |= ConvertNotificationPhysicalLocationToLocations(sarifLog);
        ...
        private static bool ConvertNotificationPhysicalLocationToLocations(JObject sarifLog)
        {
            string[] notificationPathsToUpdate =
            {
                "inlineExternalProperties[].invocations[].toolExecutionNotifications[]",
                "inlineExternalProperties[].invocations[].toolConfigurationNotifications[]",
                "inlineExternalProperties[].conversion.invocation.toolExecutionNotifications[]",
                "inlineExternalProperties[].conversion.invocation.toolConfigurationNotifications[]",
        ...
        private static bool RenameSuppressionKindValues(JObject sarifLog)
        {
            string[] suppressionPathsToUpdate =
            {
                "inlineExternalProperties[].results[].suppressions[]",
                "runs[].results[].suppressions[]",
            };
            ...
            return PerformActionOnLeafNodeIfExists(
                possiblePathsToLeafNode: suppressionPathsToUpdate,
                rootNode: sarifLog,
                action: actionOnLeaf);
        }
        ...
        private static bool HoistIdsFromPhysicalLocationToLocation(JObject sarifLog)
        {
            string[] locationPathsToUpdate =
            {
                "inlineExternalProperties[].results[].locations[]",
                "inlineExternalProperties[].results[].relatedLocations[]",
                "runs[].results[].locations[]",
                "runs[].results[].relatedLocations[]",
            };
        ...
        private static bool ApplyChangesFromTC33(JObject sarifLog)
        {
            if (sarifLog["runs"] is JArray runs)
            {
                foreach (JObject run in runs)
                {
                    ApplyChangesFromTC33ToRun(run);
                }
            }

            if (sarifLog["inlineExternalProperties"] is JArray inlineExternalProperties)
            {
                foreach (JObject externalRun in inlineExternalProperties)
                {
                    ApplyChangesFromTC33ToRun(externalRun);
                }
            }
            return true;
        }

        private static void ApplyChangesFromTC33ToRun(JObject run)
        {
        ...
        private static bool MoveSingleStackFrameAddressToLocation(JObject stackFrame)
        {
            ...
            // If 'address' is a number, move to 'absoluteAddress'
            if (stackFrame["address"] is JValue stackFrameAddress)
            {
                address.Add("absoluteAddress", stackFrameAddress);
                stackFrame.Remove("address");
            }

            // Move 'offset' to 'offsetFromParent'
            if (stackFrame["offset"] is JValue stackFrameOffset)
            {
                address.Add("offsetFromParent", stackFrameOffset);
                stackFrame.Remove("offset");
            }

            // If 'address' was already an object, just move into Location.PhysicalLocation
            if (address.Count == 0 && stackFrame["address"] is JObject addressObject)
            {
                stackFrame.Remove("address");
                address = addressObject;

                // If 'address' is a number, move to 'absoluteAddress'
                if (address["baseAddress"] is JValue baseAddress)
                {
                    address.Add("absoluteAddress", baseAddress);
                    address.Remove("baseAddress");
                }

                // Move 'offset' to 'offsetFromParent'
                if (address["offset"] is JValue offset)
                {
                    address.Add("offsetFromParent", offset);
                    address.Remove("offset");
                }
            }
        ...
        private static bool ApplyCoreTransformations(JObject sarifLog)
        {
            ...
            if (sarifLog["runs"] is JArray runs)
            {
                foreach (JObject run in runs)
                {
                    modifiedLog |= ApplyCoreTransformationsToRun(run);
                }
            }

            if (sarifLog["inlineExternalProperties"] is JArray inlineExternalProperties)
            {
                foreach (JObject inlineRun in inlineExternalProperties)
                {
                    modifiedLog |= ApplyCoreTransformationsToRun(inlineRun);
                }
            }
        ...
        private static bool ApplyCoreTransformationsToRun(JObject run)
        {
            bool modifiedLog = false;
