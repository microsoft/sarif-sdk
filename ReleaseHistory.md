...
* BRK: `multitool add-reporting-descriptor --rules` now gates the descriptor `id` against the full `NOVEL-` grammar (`NOVEL-` followed by a lowercase-alphanumeric kebab sub-id) rather than the `NOVEL-` prefix alone, so a rule descriptor `id` is byte-identical to the result `ruleId` that references it; bare `NOVEL-`, slashes, uppercase tails, and trailing hyphens are rejected on receipt.
