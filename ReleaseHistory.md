...
* NEW: Add `CweGenerateSample.ps1 -CheckedIn` switch that writes a deterministic `CweSample.sarif` next to the script (stable SRCROOT, pre-stamped notification `timeUtc`, suppressed progress output). The fixture is byte-guarded by `CweGeneratedSampleTests` and shows what enriched CWE output looks like without building anything. Convention: every taxonomy's `<Tax>GenerateSample.ps1` produces a `<Tax>Sample.sarif` fixture.
