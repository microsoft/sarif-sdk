...
* NEW: `multitool emit-init-run` now enriches `run.versionControlProvenance` from CI pipeline environment variables. Under Azure DevOps (`TF_BUILD=True`), `BUILD_REPOSITORY_URI` supplies `repositoryUri`, `BUILD_SOURCEVERSION` supplies `revisionId`, and `BUILD_SOURCEBRANCH` supplies `branch`. Under GitHub Actions (`GITHUB_ACTIONS=true`), `GITHUB_SERVER_URL` + `GITHUB_REPOSITORY` compose `repositoryUri`, `GITHUB_SHA` supplies `revisionId`, and `GITHUB_REF` supplies `branch`. All values pass through verbatim — no normalization or stripping of `refs/<class>/` prefixes (GHAzDO ingestion accepts both `main` and `refs/heads/main` shapes). When both env sets are populated, every field must agree across sources; the verb fails with a diagnostic naming both sources on any cross-source disagreement. The verb synthesizes a new entry when no `versionControlProvenance` is supplied (and `repositoryUri` is set by either source), enriches missing fields on a single supplied entry (failing on field-by-field conflict against the env-detected values), and leaves a multi-entry `versionControlProvenance` array untouched. Adds `Microsoft.CodeAnalysis.Sarif.Multitool.GitHubActionsContext`.
...
* BUG: `SARIF1001.RuleIdentifiersMustBeValid` now exempts AI-origin runs (`properties["ai/origin"]` set) from the case-fold-equality check on notification descriptors, mirroring `SARIF2009.ConsiderConventionalIdentifierValues`. Strict-identical `id`/`name` pairs continue to fire universally (SARIF v2.1.0 §3.49.7 MUST), and the case-fold-equality convention still fires for rules and taxa regardless of run origin. AI notification taxonomies pairing a `SCREAMING-CAPS` opaque id with the corresponding `PascalCase` end-user name (e.g. `id="DECISION", name="Decision"`) no longer trip the rule when carried on `tool.driver.notifications[]` of an AI-origin run.
...
* NEW: Add `Microsoft.CodeAnalysis.Sarif.Taxonomies.CweTaxonomyEnricher.Enrich(Run, CweStatus)` — fills empty `name`/`shortDescription`/`fullDescription`/`helpUri`/`help` on `reportingDescriptor` instances whose `id` matches `CWE-N`; producer-supplied fields are preserved.
