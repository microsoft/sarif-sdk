...
* NEW: Add AI emit-profile input schemas for the `add-notification`, `add-invocation`, and `add-reporting-descriptor` verbs (notification, invocation, reporting-descriptor, and `NOVEL-`-gated rule-descriptor), each pinned to its C# receipt contract by a drift test.
