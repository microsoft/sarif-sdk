...
* NEW: Introduce `Sarif.Mcp.Server` package — a `DotnetTool` (command `sarif-mcp-server`) that hosts Model Context Protocol (MCP) tools for *emitting* SARIF (create-run, start/end-invocation, add-result, add-notification, finalize). Consume-side operations (merge, transform, etc.) deferred to subsequent releases when they ship with proper test coverage. The 4.7.0 release lands the emit surface as a stub-to-real foundation; per-tool acceptance hardening follows.
* NEW: Introduce `src/Sarif/Taxonomies/CWE.sarif` — a SARIF 2.1.0 taxonomy file (per §3.19) containing 944 entries from MITRE's CWE View-1000 (Research Concepts), including names, descriptions, status, and abstraction. SARIF producers can reference the taxonomy directly via `run.taxonomies[]` or `externalPropertyFileReferences.taxonomies[]`. `Sarif.Mcp.Server.CweNameResolver` now loads this taxonomy from an embedded resource at first use and serves CWE → PascalCase-name lookups offline; the prior live-HTTP-fetch path against cwe.mitre.org is gone. Regenerate via `scripts/Generate-CweTaxonomy.ps1` when MITRE publishes a new CWE catalog version.
* NEW: `Sarif.Mcp.Server.Tools.CreateRunTool` now accepts an `allowOverwrite` parameter (default `false`). When the output path already exists and `allowOverwrite` is `false`, `sarif_create_run` fails fast with `InvalidOperationException` before registering the run; prevents an accidental second invocation from clobbering a prior scan's results.
* BUG: `Sarif.Mcp.Server.Tools.SarifLogWriter.Save` now writes atomically: the SARIF payload is serialized to a sibling `<outputPath>.tmp-<guid>` file, then `File.Move` overwrites the target. Readers observe either the prior contents or the new contents — never a partial write. On serializer failure the temp file is best-effort cleaned and the target is left untouched. Replaces the prior `File.Create(outputPath)` → in-place stream path, which left a partial file on any mid-write fault.
* NEW: Add `AI1015.RequireResultRankOnAIRun` (error) — when a run declares `ai/origin` on `run.properties` (per `AI1006`, this is an AI-emitted run), every `result` MUST carry `result.rank`. Confidence is first-class on AI findings: it drives triage priority and downstream filtering. Without `rank` the consumer has no signal of how strongly the producer believes in the result. Does not fire on non-AI runs (gated on `ai/origin` presence); `AI2010` continues to cover the SARIF-general note-level recommendation. AI* rule count 19→20.
