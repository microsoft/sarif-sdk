...
* NEW: Add `multitool add-result` and `multitool add-notification` verbs that append a fully-formed SARIF `result` (or `notification`) object — supplied as JSON via `--input <path>` or piped on stdin — to the staged event log. Payloads are parsed with `DateParseHandling.None` so ISO-8601 strings in `properties` bags and exception traces are preserved as strings without normalization. Stdin is decoded as UTF-8 regardless of the console's current code page. `add-result` validates the result's `ruleId` against the AI ruleId convention at receipt and prints an AI-consumable error envelope (code `AI-RULEID-001`) on rejection WITHOUT polluting the event log, so an AI orchestrator can correct an individual offender and retry. The JSON-payload contract preserves arbitrary nested SARIF structure (code flows, thread flows, stacks, fixes, taxa, properties bags) without SDK model normalization. `add-notification` is symmetric for notifications; producer-supplied `timeUtc` stamps are preserved, otherwise the replayer fills in `DateTime.UtcNow`.
* NEW: `Microsoft.CodeAnalysis.Sarif.Emit.AIRuleIdConvention` enforces the AI result-id grammar: every emitted `result.ruleId` must take one of two shapes — taxonomy sub-id form `BASE/sub-id` (e.g. `CWE-89/kql-injection-from-config`) or NOVEL escape hatch `NOVEL-<sub-id>` (e.g. `NOVEL-prompt-injection-via-system-message`). Validated by `SarifEventReplayer` before any descriptor registration and surfaced through `add-result` and `emit-finalize` as `AIRuleIdConventionException` (error code `AI-RULEID-001`). NOVEL- is mutually exclusive with the taxonomy form, so `NOVEL-foo/bar` is rejected. The `AI1012.ProvideRuleSubId` validation rule recognizes the NOVEL- prefix as an inherent sub-classifier and stays silent on conformant NOVEL- ruleIds. See `docs/AI-RuleId-Convention.md`.
