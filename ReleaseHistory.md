...
## **UNRELEASED**
* BRK: Hashing API consolidated around a new `HashAlgorithms` `[Flags]` enum (`Sha1`, `Sha256`, `GitBlobSha1`, plus `Default = Sha256`). SHA-1 is no longer included in default hash emission; callers that need SHA-1 must opt in by passing `HashAlgorithms.Sha1` (alone or combined). Affected public surface:
  ...
  - `HashUtilities` collapses its overload set: the parameterless-algorithm shims `ComputeHashes(string, IFileSystem)`, `ComputeHashes(Stream)`, and `ComputeHashesForText(string)` are removed; the remaining overloads accept an optional `HashAlgorithms algorithms = HashAlgorithms.Default`. `MultithreadedComputeTargetFileHashes` also gains an optional `HashAlgorithms algorithms = HashAlgorithms.Default` parameter.
  - `HashData` gains a third constructor parameter `string gitBlobSha1` and a corresponding `GitBlobSha1` property. The previous two-argument constructor remains available and leaves `GitBlobSha1` null. `ToDictionary()` emits `git-blob-sha-1` (lowercase hex; SHA-1 and SHA-256 remain uppercase per prior behavior).
