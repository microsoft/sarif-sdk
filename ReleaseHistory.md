...
* NEW: Add AI emit-profile input schemas for the `add-notification`, `add-invocation`, and `add-reporting-descriptor` verbs (notification, invocation, reporting-descriptor, and a novel-rule-descriptor overlay that `$ref`s the reporting-descriptor and adds the `--rules` `NOVEL-` id gate), each pinned to its C# receipt contract by a drift test.
