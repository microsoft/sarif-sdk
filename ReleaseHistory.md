...
* NEW: `sarif emit-finalize` now stamps a `description` on each minted `originalUriBaseIds` entry; its `text` names the repository URI and pinned commit, and its `markdown` links the short repository name to a root-at-revision URL (GitHub `/tree/<sha>`, Azure DevOps `?version=GC<sha>`). A producer-supplied description is preserved.
