...
* NEW: Validation rule `AI1016.MapToCweWeaknessNotCategory` (Error, `AI` profile) reports a `result.ruleId` whose base CWE is not a MITRE Weakness, naming the Category when it recognizes one (e.g. `CWE-16` "Configuration") so the producer maps to the specific Weakness under it rather than the grouping.
