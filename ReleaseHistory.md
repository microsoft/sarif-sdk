...
* NEW: Every CWE taxon now carries `fullDescription` (verbatim MITRE Description + Extended Description) and a `help.text` plaintext twin of `help.markdown`; `shortDescription` is the first sentence of the Description and is omitted when a consumer can recover it from `fullDescription` (SARIF §3.49.10), shrinking the taxonomy.
