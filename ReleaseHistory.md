...
* NEW: Add publishable AI emit-profile input schemas for the `add-notification`, `add-invocation`, and `add-reporting-descriptor` verbs, plus a novel-rule-descriptor overlay pinning the `--rules` `NOVEL-` id gate, each tied to its C# receipt contract by a drift test.
