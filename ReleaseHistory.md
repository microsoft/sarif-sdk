...
* BRK: `multitool emit-finalize` now requires each run to carry at least one `versionControlProvenance` entry whose `mappedTo.uriBaseId` names a declared `originalUriBaseIds` root; GitHub and Azure DevOps repository URIs are supported.
...
* NEW: `multitool emit-finalize` derives portable roots for GitHub `<slug>.ghe.com` (data-residency / Enterprise Managed Users) hosts alongside `github.com`; the GitHub host set is an allow-list and custom-hostname GitHub Enterprise Server deployments are rejected.
* NEW: `multitool emit-finalize` normalizes GitHub SSH and scp-style `repositoryUri` clone URLs (`git@github.com:owner/repo.git`, `ssh://git@github.com/owner/repo`) to https before deriving a root; Azure DevOps SSH URLs are rejected with a pointer to the https clone URL.
* NEW: `multitool emit-finalize` rejects any `versionControlProvenance[].repositoryUri` that carries embedded credentials (`account@` or `account:password@`) and normalizes GitHub ssh/scp clone URLs to their https identity so a credential-bearing or non-https URL never ships in the finalized run; the legacy `<org>.visualstudio.com` Azure DevOps host is rejected in favor of `dev.azure.com`.
* NEW: `multitool emit-run` strips any userinfo from a `BUILD_REPOSITORY_URI` it reads from the Azure DevOps pipeline environment before stamping `versionControlProvenance[].repositoryUri`, so a legacy `<org>@dev.azure.com` or PAT-bearing clone URL yields a clean identity rather than failing the credential-rejection gate.
* NEW: `multitool emit-run` validates at receipt that every `versionControlProvenance[].repositoryUri` has a shape from which `emit-finalize` can derive a portable root, failing on an unsupported host or malformed path.
