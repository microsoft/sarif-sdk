...
* NEW: Validation rule `AI1016.ProvideValidRuleId` (Error, `AI` profile) reports a `tool.driver.rules[]` descriptor id that is neither a CWE Weakness nor a `NOVEL-<sub-id>` id — a Category (named when recognized, e.g. `CWE-16` "Configuration"), a View, a withdrawn or unknown CWE, or a non-CWE token such as a MITRE ATT&CK technique.
* NEW: Public `Microsoft.CodeAnalysis.Sarif.Taxonomies.CweTaxonomy.IsKnownWeakness` and the new `CweCategories` type (`TryGetCategoryName`, `IsCategory`, `CategoryNamesByCwe`) expose the embedded MITRE CWE Category id→name map and the Weakness-membership oracle.
...
* BUG: `emit-finalize` floors a CWE rule descriptor's `name` to its id only when the CWE is a known MITRE Weakness (e.g. under `--no-cwe-enrichment`), so the descriptor stays `GHAzDO2012`-publishable; a Category, View, or unknown CWE such as `CWE-16` is left nameless so the producer mapping bug fails loudly (`AI1016`, `GHAzDO2012`) instead of being normalized.
