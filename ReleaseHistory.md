...
* BUG: `SarifLogger` no longer throws when a `run.tool.extensions[]` entry omits the optional `toolComponent.guid`; the guidless extension is left out of `ExtensionGuidToIndexMap` while its rules are still recorded.
...
* NEW: `@microsoft/sarif-multitool-ts` ships `CweCategories.json` and exports `getCweCategories`, `isCweCategory`, and `getCweCategoryName`, mirroring the .NET `CweCategories` helper so a consumer can recognize a CWE Category id (e.g. `CWE-16`) that is never a valid `result.ruleId` mapping target.
