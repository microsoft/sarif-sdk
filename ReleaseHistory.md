...
* NEW: `AI1012.ProvideRuleSubId` now flags a malformed AI ruleId as `Error_Malformed`, distinct from a bare `CWE-<n>` (still `Error_Missing`); a bare `CWE-<n>` on a GitHub-hosted run is accepted, since that is the expected shape after the `emit-finalize` sub-id collapse.
...
* NEW: `Microsoft.CodeAnalysis.Sarif.Taxonomies.CweSecuritySeverity.TryGetCweNumber` parses the CWE number from any conformant id (`89`, `CWE-89`, `CWE-89/<slug>`); the `NOVEL-` form yields `false`.
