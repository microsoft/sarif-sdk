...
* BRK: Strip editorial prefixes from the AI notification taxonomy. Descriptor ids now name the concern only — `DECISION`, `RULED-OUT`, `DATA-ACCESS-DENIED`, `ALAS-SIGNAL`, `TOOL-UNAVAILABLE`, etc. The previous `AI/EXEC/*` and `AI/CFG/*` prefixes were redundant: the surrounding array (`toolExecutionNotifications` vs `toolConfigurationNotifications`) already encodes the kind, and `tool.driver.name` already encodes the emitter. The same id MAY now legally appear in both arrays. Placement is selected at authoring time: `add-notification` defaults to `toolExecutionNotifications`, and a new `--config` (`-c`) switch routes to `toolConfigurationNotifications`. The event-log kind `SarifEventKinds.Notification` (value `"notification"`) splits into `SarifEventKinds.ExecutionNotification` (`"execution-notification"`) and `SarifEventKinds.ConfigurationNotification` (`"configuration-notification"`); the replayer routes each to the matching array. Removes `AI1014.ExecutionNotificationPlacement` (added in v4.6.3): its sole purpose was enforcing the now-dropped prefix-vs-array consistency, which has no analogue under the new convention. `AI2018.ProvideExecutionSignalArtifact` retains its semantic; the literal id it checks changes from `AI/EXEC/ALAS-SIGNAL` to `ALAS-SIGNAL`.
