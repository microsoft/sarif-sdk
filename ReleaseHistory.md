...
* NEW: `emit-init-run` auto-detects an Azure DevOps pipeline execution context from the standard ADO predefined environment variables and stamps the corresponding `automationDetails` shape — `id` (canonical `azuredevops/pipeline/build/<org>/<projectId>/<buildDefId>/<phaseId>/<branchRef>/<buildId>` form) plus the four `azuredevops/pipeline/build/*` property keys validated by GHAzDO1019. Gated on `TF_BUILD=True`; reads `SYSTEM_COLLECTIONURI`, `SYSTEM_TEAMPROJECTID`, `BUILD_DEFINITIONID` (with `SYSTEM_DEFINITIONID` as the fallback; both must agree if set), `BUILD_DEFINITIONNAME`, `BUILD_BUILDID`, `SYSTEM_PHASEID` (with `SYSTEM_JOBID` as the fallback; primary wins when both are set), `SYSTEM_PHASENAME` (with `SYSTEM_JOBNAME` fallback), and `BUILD_SOURCEBRANCH`. Detection is three-state: when no required variables are set, no stamping occurs; when all are set and well-formed, stamping happens before any file-system side effects; when partially set or malformed, the verb fails loudly with a per-variable diagnostic so a misconfigured pipeline never emits a half-stamped SARIF that would fail GHAzDO1019/1020 downstream. To opt out in a CI context, clear the listed environment variables before invoking the verb. Pre-existing `--automation-guid` / `--automation-correlation-guid` flags compose with the env-driven path; an env-stamped `automationDetails.id` does not overwrite a producer-supplied `guid`/`correlationGuid`. Adds `Microsoft.CodeAnalysis.Sarif.Multitool.AdoPipelineContext`.
* NEW: Add `CweGenerateSample.ps1 -GHAzDO` and the deterministic `CweGHAzDoSample.sarif` fixture it produces. The new fixture sits alongside the existing `CweSample.sarif` (AI-shape, validated under `--rule-kind Sarif;AI`) and represents the same AI scanner running inside an Azure DevOps pipeline — the script populates the ADO predefined environment variables for the duration of `emit-init-run` so `AdoPipelineContext` stamps `run.automationDetails.id` plus the four `azuredevops/pipeline/build/*` properties, and patches `tool.driver.fullName` post-finalize so `GHAzDO1018` passes. Validated under `--rule-kind Sarif;AI;GHAzDO` with zero errors, zero warnings, and zero notes. `CweGeneratedSampleTests` covers both fixtures with byte-identical regression gates.
