...
## **v5.0.3** UNRELEASED
* BRK: Split AI rule `AI2011 DoNotPersistFingerprints` (Note) into `AI1007 DoNotPersistFingerprints` (Error; forbids `result.fingerprints`) and `AI2011 DoNotPersistPartialFingerprints` (Warning; `result.partialFingerprints`). `RuleId.AIDoNotPersistFingerprints` now maps to `AI1007`; new `RuleId.AIDoNotPersistPartialFingerprints` maps to `AI2011`.
...
* BRK: Rename the embedded CWE-taxonomy assets `CWE.sarif` / `CWE.brief.md` to `CweTaxonomy.sarif` / `CweTaxonomy.brief.md`; the manifest LogicalName and `CweTaxonomy.SarifResourceName`/`BriefResourceName` constants change accordingly. The taxonomy now emits each `taxon.name` as a strict Pascal-case identifier conformant with `SARIF2012` (e.g. `CrossSiteScripting`, `SqlInjection`, `UseOfHardCodedCredentials`), and preserves the catalog's original long title in a new `taxon.properties["cwe/title"]` bag so the human-readable name isn't lost.
* BRK: Rename the `Ado` ruleset to `GHAzDO` (precise casing: `GH` + `Az` + `DO`, reflecting "GitHub Advanced Security for Azure DevOps"). The `RuleKind.Ado` enum value becomes `RuleKind.GHAzDO`; rule IDs `ADO1011`–`ADO1018` and `ADO2012` become `GHAzDO1011`–`GHAzDO1018` and `GHAzDO2012`; rule class names follow (`AdoProvideRequiredResultProperties` → `GHAzDOProvideRequiredResultProperties`, etc.); resource keys, `RuleId.*` constants, and prose ("ADO Advanced Security service" → "GHAzDO (GitHub Advanced Security for Azure DevOps) service") update accordingly. Existing suppression files and policy configurations referencing the old IDs must be updated.
...
* NEW: SARIF validation rules `SARIF2002`, `SARIF2009`, `SARIF2014`, and `SARIF2015` now self-suppress when the containing run declares an `ai/origin` marker (any non-empty `run.properties["ai/origin"]` value triggers suppression). These four style-class rules give human-author guidance that does not apply to AI-emitted content: AI scanners render `message.text` per-result rather than via `message.id`+`arguments` (SARIF2002), use `NOVEL-` ids as the documented escape hatch from conventional ruleId forms (SARIF2009), and ship rule descriptors with direct messages rather than `{N}` placeholders (SARIF2014/2015). Correctness-class rules (SARIF2003, 2006, 2010, 2011, 2012, 2013, 2016) and the ADO ingestion rules continue to apply uniformly. Adds `SarifValidationSkimmerBase.IsAIOriginRun(Run)` as the shared detection helper.
