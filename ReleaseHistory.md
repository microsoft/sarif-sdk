...
* NEW: `--insert RollingHashPartialFingerprints` (also in `All`) stamps each result's `primaryLocationLineHash` partial fingerprint using the CodeQL rolling line-hash, consumed by GitHub code scanning and GHAzDO for result matching. Results lacking a primary-location `startLine` are skipped.
