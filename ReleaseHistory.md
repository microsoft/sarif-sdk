...
* NEW: Add `multitool add-result` and `multitool add-notification` verbs that append a fully-formed SARIF `result` (or `notification`) object — supplied as JSON via `--input <path>` or piped on stdin — to the staged event log. `add-result` validates the result's `ruleId` against the AI ruleId convention at receipt and prints an AI-consumable error envelope (code `AI-RULEID-001`) on rejection WITHOUT polluting the event log, so an AI orchestrator can correct an individual offender and retry. The JSON-payload contract preserves arbitrary nested SARIF structure (code flows, thread flows, stacks, fixes, taxa, properties bags) verbatim. `add-notification` is symmetric for notifications; producer-supplied `timeUtc` stamps are preserved, otherwise the replayer fills in `DateTime.UtcNow`.
