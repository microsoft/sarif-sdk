...
* NEW: `@microsoft/sarif` ships the native-TypeScript SARIF 2.1.0 SDK: open-typed object model, `FileRegionsCache` region/snippet resolution, `AIRuleIdConvention`, rolling-hash and optional-data enrichment, and a `serializeSarifLog` whose output is byte-for-byte identical to the .NET multitool; no CLR dependency.
* NEW: `@microsoft/sarif-multitool-ts` ships the in-process `emit-run`/`emit-results`/`emit-invocations`/`emit-rule-descriptors`/`emit-notification-descriptors`/`emit-finalize`/`get-schema`/`get-skill`/`get-cwe` verbs and an arg-compatible `sarif` CLI, replacing the v5.1.1 publish-proof shells; no CLR dependency.
