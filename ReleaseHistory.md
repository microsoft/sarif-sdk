...
* NEW: The plural `add-*` verbs accept a single JSON object or a JSON array and validate the payload atomically: if any element is rejected nothing is appended, and the outcome is reported as `{ "appended": N, "rejected": [ { "index", "errorCode", "message" } ] }` on stdout (intra-batch duplicate ids are rejected alongside cross-log duplicates).
