...
* NEW: `sarif emit-finalize` now stamps a `description` on each minted `originalUriBaseIds` entry whose `text` is a SARIF embedded link (§3.11.6) linking the short repository name to a root-at-revision URL (GitHub `/tree/<sha>`, Azure DevOps `?version=GC<sha>`) and naming the pinned commit. A producer-supplied description is preserved.
