...
* BUG: `ZipArchiveArtifact.Stream` returns an independent in-memory snapshot of the entry's bytes instead of a live view over the shared archive, so concurrent reads of one archive's entries no longer race and `Stream` works regardless of `Contents`/`Bytes` access order.
