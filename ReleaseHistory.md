...
* NEW: `sarif emit-finalize --no-repo` finalizes a scan that has no `versionControlProvenance`: it enriches as usual, elides the transient local `originalUriBaseIds` root instead of rebasing it, and stamps each run `properties.unpublishable = true`, which `publish-to-ghazdo` refuses.
