...
Each release entry below is prefixed with one of:
...
- `FUN` => Fundamentals clean-up or other work.
...
* BRK: the whole-log schema served by `get-schema emit-finalize` is renamed `ai-log.schema.json` → `ai-sarif-log.schema.json` (matching the SARIF `sarifLog` type, as the other `ai-*` schemas are named for their SARIF object); update any direct `@microsoft/sarif-multitool-ts/schemas/ai-log.schema.json` import or `$id` reference.
* BUG: `get-schema emit-finalize` in the `@microsoft/sarif-multitool-ts` package now returns the whole-log schema instead of throwing; the TypeScript verb→schema map was missing the entry the .NET multitool already serves.
...
* NEW: Validation rule `AI1016.ProvideValidRuleId` (Error, `AI` profile) requires every `tool.driver.rules[]` id to be a CWE Weakness or a `NOVEL-<sub-id>` id, with a tailored diagnostic that names a CWE Category when one is used (e.g. `CWE-16` "Configuration").
* NEW: Public `Microsoft.CodeAnalysis.Sarif.Taxonomies.CweTaxonomy.IsKnownWeakness` and the new `CweCategories` type expose the embedded MITRE CWE Weakness-membership oracle and Category id→name map.
* NEW: `get-schema emit-finalize` serves `ai-log.schema.json`, the finalized whole-log AI contract — accepts finalize-assigned `result.ruleIndex`/`guid` and enforces the AI Error-level rules plus the per-run GHAzDO `automationDetails` shape. Completes the verb→schema map (6 of 6).
* BUG: `Run.GetFileIndex` no longer materializes a `sarif:`-scheme pointer URI (an in-log result address used by `relatedLocations` cross-links and partitioning) as a phantom entry in `run.artifacts[]`; this affects `emit-finalize` and any file-reference pass.
* BUG: `emit-run` leaves a single `versionControlProvenance` entry untouched when its `repositoryUri` names a different repository than the pipeline enlistment, instead of reconciling it against (and failing on) the pipeline `revisionId`/`branch`.
* BUG: when a CWE rule descriptor has no `name`, `emit-finalize` floors it to the id only if the CWE is a known MITRE Weakness; a Category, View, or unknown CWE is left nameless rather than normalized.
...
* NEW: `project` verb projects results into flat `csv`/`tsv`/`ndjson` rows over an ordered `--columns` list; `Level` inherits `defaultConfiguration.level` then `warning`, and `Properties.<key>` (e.g. `security-severity`) falls back to `rule.properties`; one row per location.
* NEW: `sarif emit-finalize --no-repo` finalizes a scan that has no `versionControlProvenance`: it enriches as usual, elides the transient local `originalUriBaseIds` root instead of rebasing it, and stamps each run `properties.unpublishable = true`, which the publish verbs refuse.
* NEW: `publish-to-ghas` verb uploads a finalized SARIF to GitHub code scanning, deriving the target, commit, and ref from `versionControlProvenance` and reading the bearer token from the env var named by `--token-env-var` (default `GHAS_TOKEN`); refuses an unpublishable (`--no-repo`) log.
* NEW: `suppress` gains `--expression` to suppress every result matching a query expression and `--results-guids` to suppress results by `guid`; already-suppressed, non-expired results are left untouched.
* NEW: `query` adds an `IsSuppressed` expression that matches results carrying an active (non-expired) suppression, letting auditors filter by suppression status.
...
* BUG: `SARIF2006.UrisShouldBeReachable` no longer raises a skimmer exception when a URI such as `downloadUri` or `helpUri` uses a non-HTTP(S) scheme (e.g. `ftp:`); the rule probes only `http`/`https` URIs, the only schemes an HTTP GET can service.
...
* NEW: `sarif emit-finalize` collapses each result's hierarchical `ruleId` (and `result.rule.id`) to its `reportingDescriptor.id` for GitHub-hosted runs, so GitHub's classifier — which binds by `ruleId`-string equality — attaches `security-severity` and `tags`. Azure DevOps keeps the sub-id.
* NEW: `sarif emit-finalize` now stamps a neutral medium `security-severity` (`5.0`) on uncurated content — a CWE absent from `CweSecuritySeverity` or a `NOVEL-` rule — so every AI security rule carries a severity; a curated prior or producer value still wins.
* NEW: `AI1012.ProvideRuleSubId` accepts a bare `CWE-<n>` on a GitHub-hosted run (still `Error_Missing` on every other host), since that is the expected shape after the `emit-finalize` sub-id collapse.
* NEW: `Microsoft.CodeAnalysis.Sarif.Taxonomies.CweSecuritySeverity.TryGetCweNumber` parses the CWE number from a prefixed id (`CWE-89`, any case) or AI ruleId sub-id form (`CWE-89/<slug>`); a bare number (`89`), the `NOVEL-` form, and non-conforming ids yield `false`.
...
* NEW: `AIRuleIdConvention.TryGetCweId` extracts the canonical `CWE-<n>` from any conformant AI ruleId (`CWE-89`, `CWE-89/<slug>`).
...
* NEW: `sarif merge --merge-runs` now aggregates `run.invocations` across merged runs and rebases each result's `provenance.invocationIndex`, so result→invocation traceability survives the merge; invocations were previously discarded.
...
* NEW: `sarif emit-finalize` `versionControlProvenance.mappedTo` diagnostics are now instructive: a missing `uriBaseId` names the binding to use (conventionally `SRCROOT`) and shows the fix; a base that does not resolve to a local `file://` checkout explains the transient resolution and rebase.
* NEW: `--insert RollingHashPartialFingerprints` (also in `All`) stamps each result's `primaryLocationLineHash` via the CodeQL rolling line-hash, consumed by GitHub code scanning and GHAzDO for result matching. A region-less result is hashed from line 1, per github/codeql-action.
* NEW: The emit-time AI-ruleId rejection now reports the canonical `AI1012.ProvideRuleSubId` error code (shared with the validator) and names the fallback for a bare `CWE-<n>` ruleId: kebab-case the CWE name as the sub-id (e.g., `CWE-89/sql-injection`), authored by the producer.
* NEW: `sarif emit-finalize` now derives each rule's GHAS `security-severity` from the highest `result.rank` among the results referencing it (rank 0–100 scaled to 0.0–10.0); a rule with no ranked result, and any producer-authored value, are left untouched.
* NEW: `sarif get-cwe` serves canonical MITRE CWE data from the embedded taxonomy as JSON (default) or markdown (`--format`), for named ids (`89`, `CWE-89`) or `--all`; `--concise` drops the full help markdown. Each record's `ruleIdFallback` (`CWE-<n>/<slug>`) matches `AI1012.ProvideRuleSubId`.
...
* BRK: `sarif add-invocation` now requires `executionSuccessful`, a non-whitespace `commandLine`, a `workingDirectory` with a non-whitespace `uri`, and a producer-supplied `timeUtc` on every inline notification; it auto-stamps only `endTimeUtc` when omitted.
* BRK: Rename `sarif emit-init-run` to `emit-run`.
...
* BRK: Remove the `--srcroot` flag from `sarif emit-finalize`; the verb now derives portable artifact roots from `run.versionControlProvenance` rather than a caller-supplied root.
* BRK: `sarif emit-finalize` now requires each run to carry at least one `versionControlProvenance` entry whose `mappedTo.uriBaseId` names a declared `originalUriBaseIds` root; GitHub and Azure DevOps repository URIs are supported.
* BRK: Split `sarif add-reporting-descriptor` into `add-notification-reporting-descriptor` and `add-rule-reporting-descriptor`, removing the `--rules` flag.
* BRK: `sarif add-rule-reporting-descriptor` now gates the descriptor `id` against the full `NOVEL-` grammar (`NOVEL-` followed by a lowercase-alphanumeric kebab sub-id) rather than the `NOVEL-` prefix alone.
...
* NEW: `sarif emit-finalize` rebases absolute local artifact paths to portable per-repository roots derived from `versionControlProvenance`, binding one repository to `SRCROOT` and multiple to `SRCROOT_<REPO>`; it fails closed on any local path no root resolves.
* NEW: `sarif emit-finalize` derives portable roots for `dev.azure.com` repositories — the commit-less `dev.azure.com/<org>/<project>/_git/<repo>/` root, with commit pinning carried on `versionControlProvenance.revisionId` — alongside `github.com` blob permalinks.
* NEW: `sarif emit-run` stamps `versionControlProvenance[0].mappedTo.uriBaseId = "SRCROOT"` when it auto-fills a single source-repo entry and the run declares an `originalUriBaseIds["SRCROOT"]` root.
* NEW: `sarif emit-finalize` derives portable roots for GitHub `<slug>.ghe.com` (data-residency / Enterprise Managed Users) hosts alongside `github.com`; the GitHub host set is an allow-list and custom-hostname GitHub Enterprise Server deployments are rejected.
* NEW: `sarif emit-finalize` normalizes GitHub SSH and scp-style `repositoryUri` clone URLs (`git@github.com:owner/repo.git`, `ssh://git@github.com/owner/repo`) to https before deriving a root; Azure DevOps SSH URLs are rejected with a pointer to the https clone URL.
* NEW: `sarif emit-finalize` rejects any `versionControlProvenance[].repositoryUri` carrying embedded credentials (`account@` or `account:password@`), and rejects the legacy `<org>.visualstudio.com` Azure DevOps host in favor of `dev.azure.com`.
* NEW: `sarif emit-run` strips userinfo from a `BUILD_REPOSITORY_URI` read from the Azure DevOps pipeline environment before stamping `versionControlProvenance[].repositoryUri`, so a PAT-bearing or `<org>@dev.azure.com` clone URL yields a clean identity.
* NEW: `sarif emit-run` validates at receipt that every `versionControlProvenance[].repositoryUri` has a shape from which `emit-finalize` can derive a portable root, failing on an unsupported host or malformed path.
...
* NEW: `multitool emit-init-run` populates `run.versionControlProvenance` (`repositoryUri`, `revisionId`, `branch`) from Azure DevOps (`TF_BUILD`) and GitHub Actions (`GITHUB_ACTIONS`) CI environment variables; cross-source disagreement fails the verb.
...
* BUG: `SARIF1001.RuleIdentifiersMustBeValid` now exempts AI-origin runs (`properties["ai/origin"]` set) from the case-fold-equality check on notification descriptors, matching `SARIF2009.ConsiderConventionalIdentifierValues`. Strict-identical `id`/`name` pairs still fire universally.
...
* BRK: Hashing API consolidated around a new `HashAlgorithms` `[Flags]` enum (`Sha1`, `Sha256`, `Sha512`, `GitBlobSha1`, plus `Default = Sha256`). SHA-1 is no longer included in default hash emission; callers that need SHA-1 must opt in by passing `HashAlgorithms.Sha1` (alone or combined). Affected public surface:
  ...
  - `Artifact.Create` accepts a new `HashAlgorithms hashAlgorithms = HashAlgorithms.Default` parameter (added at the end). The method now suppresses emission of an empty `"hashes": {}` object when no algorithm produces a value (e.g., `HashAlgorithms.None`, or an all-null `HashData`).
  - `Run.GetFileIndex` accepts new `IFileSystem fileSystem = null` and `HashAlgorithms hashAlgorithms = HashAlgorithms.Default` parameters (both added at the end), threaded to `Artifact.Create`. When a caller flows a non-default `IFileSystem` through `Artifact.Create` (e.g., a mock for testing, or a sandboxed adapter), it now reaches the hash-computation path consistently.
  - `HashUtilities` collapses its overload set: the parameterless-algorithm shims `ComputeHashes(string, IFileSystem)`, `ComputeHashes(Stream)`, and `ComputeHashesForText(string)` are removed; the remaining overloads accept an optional `HashAlgorithms algorithms = HashAlgorithms.Default`. `MultithreadedComputeTargetFileHashes` also gains optional `IFileSystem fileSystem = null` and `HashAlgorithms algorithms = HashAlgorithms.Default` parameters.
  - `HashData` is modernized: positional constructors are removed in favor of object-initializer syntax (`new HashData { Sha256 = "...", GitBlobSha1 = "..." }`); the existing properties are now settable. A new `Sha512` property is added; `ToDictionary()` emits the `sha-512` key when set. `git-blob-sha-1` is lowercase hex; SHA-1, SHA-256, and SHA-512 are uppercase hex (consistent with prior SHA-1 / SHA-256 behavior).
* BRK: Rename the embedded CWE-taxonomy assets `CWE.sarif` / `CWE.brief.md` to `CweTaxonomy.sarif` / `CweTaxonomy.brief.md`; the manifest LogicalName and `CweTaxonomy.SarifResourceName`/`BriefResourceName` constants change accordingly. The taxonomy now emits each `taxon.name` as a strict Pascal-case identifier conformant with `SARIF2012.ProvideRuleProperties` (e.g. `CrossSiteScripting`, `SqlInjection`, `UseOfHardCodedCredentials`), and preserves the catalog's original long title in a new `taxon.properties["cwe/title"]` bag so the human-readable name isn't lost.
* BRK: Rename the `Ado` ruleset to `GHAzDO` (precise casing: `GH` + `Az` + `DO`, reflecting "GitHub Advanced Security for Azure DevOps"). The `RuleKind.Ado` enum value becomes `RuleKind.GHAzDO`; rule IDs `ADO1011.ReferenceFinalSchema`–`ADO1018.ProvideRequiredToolProperties` and `ADO2012.ProvideRequiredReportingDescriptorProperties` become `GHAzDO1011.ReferenceFinalSchema`–`GHAzDO1018.ProvideRequiredToolProperties` and `GHAzDO2012.ProvideRequiredReportingDescriptorProperties`; rule class names follow (`AdoProvideRequiredResultProperties` → `GHAzDOProvideRequiredResultProperties`, etc.); resource keys, `RuleId.*` constants, and prose ("ADO Advanced Security service" → "GHAzDO (GitHub Advanced Security for Azure DevOps) service") update accordingly. Existing suppression files and policy configurations referencing the old IDs must be updated.
* BRK: Strip editorial prefixes from the AI notification taxonomy. Descriptor ids now name the concern only — `DECISION`, `RULED-OUT`, `DATA-ACCESS-DENIED`, `LEARNING-SIGNAL`, `TOOL-UNAVAILABLE`, etc. The previous `AI/EXEC/*` and `AI/CFG/*` prefixes are removed; the same id MAY now legally appear in both notification arrays. `add-notification` defaults to `toolExecutionNotifications`; new `--config` (`-c`) switch routes to `toolConfigurationNotifications`. `SarifEventKinds.Notification` splits into `ExecutionNotification` and `ConfigurationNotification`; the replayer routes each to the matching invocation array.
* BRK: Remove `AI1014.ExecutionNotificationPlacement` — its sole purpose was the now-dropped prefix-vs-array consistency check. Rename `AI2018.ProvideExecutionSignalArtifact` to `ProvideLearningSignalArtifact`; the id literal it checks changes from `AI/EXEC/ALAS-SIGNAL` to `LEARNING-SIGNAL`.
* NEW: `HashAlgorithms.GitBlobSha1` opt-in computes a SHA-1 over `"blob " + length + "\0" + content` (the same construction `git hash-object <file>` uses), emitted under the artifact-hashes dictionary key `git-blob-sha-1`. The value is computed over the raw on-disk bytes and is therefore sensitive to line-ending configuration; matching a server-persisted git blob SHA requires the on-disk bytes to match what git stored. Producible for any seekable stream or file path; a non-seekable stream is buffered into memory as a fallback. The stream-based overload hashes from the stream's current position to its end, so the value matches `git hash-object` exactly when the stream is positioned at zero.
* NEW: `HashAlgorithms.Sha512` opt-in computes SHA-512 in a single pass alongside the other selected algorithms; emitted under the artifact-hashes dictionary key `sha-512` (uppercase hex).
* NEW: Add `Microsoft.CodeAnalysis.Sarif.Taxonomies.CweTaxonomy` — loads the embedded MITRE cwec_v4.20 catalog (969 entries) as SARIF (`Load`) or compact markdown table (`LoadBrief`); filtered by `CweStatus` flag, default `Stable | Draft | Incomplete`.
* NEW: Add `Microsoft.CodeAnalysis.Sarif.Taxonomies.CweTaxonomyEnricher.Enrich(Run, CweStatus)` — fills empty `name`/`shortDescription`/`fullDescription`/`helpUri`/`help` on `reportingDescriptor` instances whose `id` matches `CWE-NNNN`; producer-supplied fields are preserved.
* NEW: Add incremental SARIF authoring foundation in the new `Microsoft.CodeAnalysis.Sarif.Emit` namespace. Producers can author a SARIF log across multiple process invocations via an append-only JSONL event log:
  * `SarifEventLogWriter` / `SarifEventLogReader` — versioned event log (kinds: `run-header`, `result`, `invocation`, `notification`); forward-compatible reader.
  * `SarifEventReplayer` — replays an event log into a `SarifLog`, auto-registering `tool.driver.rules` and routing notifications to the last invocation.
  * `AtomicSarifWriter` — writes via a same-directory temp file + `File.Move(overwrite: true)` so readers never see a partial file.
* NEW: Add `multitool emit-init-run` and `multitool emit-finalize` verbs that bookend the incremental authoring flow. `emit-init-run` opens `<output>.sarif.wip.jsonl` with tool metadata and VCP; `emit-finalize` replays it, applies CWE enrichment (`--no-cwe-enrichment` to skip), atomically writes the final SARIF, and removes the wip (`--keep-wip` to retain).
* NEW: Add `multitool add-result` and `multitool add-notification` verbs that append a fully-formed SARIF result (or notification) JSON object — supplied via `--input <path>` or stdin — to the staged event log. `add-result` validates `result.ruleId` against `AIRuleIdConvention` at receipt; rejection prints the `AI1012.ProvideRuleSubId` code to stderr without appending. Payloads are parsed with `DateParseHandling.None` and stdin decoded as UTF-8.
* NEW: Add `Microsoft.CodeAnalysis.Sarif.Emit.AIRuleIdConvention` and `AIRuleIdConventionException` (error code `AI1012`). Every AI-emitted `result.ruleId` must take taxonomy sub-id form (`BASE/sub-id`, e.g. `CWE-89/kql-injection`) or NOVEL escape hatch (`NOVEL-<sub-id>`); the NOVEL- form does not accept a slash. `AI1012.ProvideRuleSubId` recognizes the NOVEL- prefix and stays silent on conformant ids.
* NEW: Internal — `Sarif.Multitool.Program.Main` now uses the non-generic `Parser.ParseArguments(args, params Type[])` overload to lift the 16-verb ceiling.
* NEW: Extend `SARIF2004.OptimizeFileSize` to flag explicit emission of the `-1` "unset index" sentinel (§3.4) on `index`, `ruleIndex`, and `parentIndex` — semantically equivalent to omitting the property but inflates the log. Resolves #2910.
* NEW: Add `multitool emit-finalize --srcroot <uri>` that rewrites `run.originalUriBaseIds["SRCROOT"].uri` after `InsertOptionalDataVisitor` enrichment but before serialization. Producers emit with a local `file://` SRCROOT so the visitor can read sources for snippets, contextRegion, and sha-256, then ship the SARIF with a portable canonical anchor (e.g. `https://github.com/<org>/<repo>/blob/<branch>/`).
* NEW: Add `multitool emit-finalize --embed-text-files` to fold `OptionallyEmittedData.TextFiles` into the always-on enrichment so the finalized SARIF carries embedded `artifacts[].contents.text` for textual sources. Off by default; produces self-contained logs when on.
* NEW: Add `multitool emit-finalize --validate` that runs the SARIF + AI validation rule kinds against the finalized SARIF and fails (exit FAILURE) when any error-level findings are reported. Warnings and notes are summarized but non-blocking. Pairs with `emit-finalize` to make full-log validation an opt-in gate at the producer's authoring boundary.
* NEW: Add four `emit-init-run` flags that populate run-header AI-profile metadata: `--tool-driver-semantic-version` (sets `tool.driver.semanticVersion`), `--automation-guid` and `--automation-correlation-guid` (canonical 8-4-4-4-12 GUID; populates `run.automationDetails.guid` / `correlationGuid`), and `--ai-origin` (one of `generated`/`annotated`/`synthesized`; sets `run.properties["ai/origin"]`). When `--srcroot` and `--vcp-repositoryuri` are both supplied, the versionControlProvenance entry's `mappedTo.uriBaseId` is set to `SRCROOT` (satisfies `AI1004.ProvideVersionControlProvenance`).
* NEW: Add `CweGenerateSample.ps1` + the deterministic `CweSample.sarif` fixture it produces. The script (now zero-parameter) writes the fully-enriched fixture next to itself; `CweGeneratedSampleTests` hashes before/after as the regression gate. The fixture passes `validate --rule-kind Sarif;AI --level Error;Warning;Note` with zero errors and zero warnings. Convention: every taxonomy's `<Tax>GenerateSample.ps1` produces a `<Tax>Sample.sarif` fixture.
* NEW: SARIF validation rules `SARIF2002.ProvideMessageArguments`, `SARIF2009.ConsiderConventionalIdentifierValues`, `SARIF2014.ProvideDynamicMessageContent`, and `SARIF2015.EnquoteDynamicMessageContent` now self-suppress when the containing run declares an `ai/origin` marker (any non-empty `run.properties["ai/origin"]` value triggers suppression). These four style-class rules give human-author guidance that does not apply to AI-emitted content: AI scanners render `message.text` per-result rather than via `message.id`+`arguments` (SARIF2002), use `NOVEL-` ids as the documented escape hatch from conventional ruleId forms (SARIF2009), and ship rule descriptors with direct messages rather than `{N}` placeholders (SARIF2014/2015). Correctness-class rules (`SARIF2003.ProvideVersionControlProvenance`, 2006, 2010, 2011, 2012, 2013, 2016) and the ADO ingestion rules continue to apply uniformly. Adds `SarifValidationSkimmerBase.IsAIOriginRun(Run)` as the shared detection helper.
* NEW: `--rule-kind` argument now binds case-insensitively. `--rule-kind ghazdo`, `--rule-kind GHAzDO`, and `--rule-kind GHAZDO` all parse to `RuleKind.GHAzDO`; the same applies to `sarif`/`gh`/`ghas`/`ai`.
* NEW: `GHAzDO1019.ProvidePipelineProperties` — when `run.automationDetails` is present, validates the four `azuredevops/pipeline/build/*` keys on `automationDetails.properties` that Azure DevOps Advanced Security ingestion requires: `buildDefinitionId` (integer, non-zero), `buildDefinitionName` (non-empty string), `phaseId` (GUID, not the empty GUID), and `phaseName` (non-empty string). Error level.
* NEW: `GHAzDO1020.ProvideAutomationDetailsIdFormat` — when `run.automationDetails.id` is present, validates that it starts with the canonical `azuredevops/pipeline/build/` prefix so downstream parsing of `<org>/<project>/<buildDefId>/<phaseId>/<branch>/<buildId>` succeeds. Error level.
* NEW: `emit-init-run` auto-stamps `automationDetails.id` (canonical `azuredevops/pipeline/build/...` form) and the four `azuredevops/pipeline/build/*` properties validated by `GHAzDO1019.ProvidePipelineProperties`/1020 from the standard ADO pipeline environment variables when `TF_BUILD=True`. Reads `SYSTEM_COLLECTIONURI`, `SYSTEM_TEAMPROJECTID`, `BUILD_DEFINITIONID` (with `SYSTEM_DEFINITIONID` fallback), `BUILD_DEFINITIONNAME`, `BUILD_BUILDID`, `SYSTEM_PHASEID` (with `SYSTEM_JOBID` fallback), `SYSTEM_PHASENAME` (with `SYSTEM_JOBNAME` fallback), and `BUILD_SOURCEBRANCH`. Partial or malformed env state fails the verb with a per-variable diagnostic before any file is written. Composes with `--automation-guid` / `--automation-correlation-guid`. Adds `Microsoft.CodeAnalysis.Sarif.Multitool.AdoPipelineContext`.
* NEW: `CweGenerateSample.ps1 -GHAzDO` produces `CweGHAzDOSample.sarif` — an "AI scanner in an ADO pipeline" companion to `CweSample.sarif`. Validates under `--rule-kind Sarif;AI;GHAzDO` with zero findings; `CweGeneratedSampleTests` gates both fixtures byte-identically.
* NEW: `CweGenerateSample.ps1` derives `--vcp-repositoryuri` and the `emit-finalize --srcroot` prefix from `git -C $repoRoot remote get-url origin`, falling back to `https://github.com/microsoft/sarif-sdk` when origin is unset; on the canonical clone the generated fixtures are byte-identical to the previous hardcoded form. GitHub origins get a `<repo>/blob/main/` SRCROOT; other hosts (including Azure DevOps) get the bare repo URL with a trailing slash.
* NEW: Add `src/Sarif/Taxonomies/PublishSampleToGhazdo.ps1` — POSTs a gzipped SARIF to the GHAzDO SARIFs ingestion endpoint (`/{org}/{project}/_apis/alert/repositories/{repo}/sarifs?api-version=7.2-preview.1` on `advsec.dev.azure.com`, fallback `dev.azure.com`) with `Content-Type: application/octet-stream` and no `Content-Encoding` header. Target organization/project/repository are parsed from `runs[0].versionControlProvenance[0].repositoryUri` (supports `https://dev.azure.com/{org}/{project}/_git/{repo}` and `https://{org}.visualstudio.com/{project}/_git/{repo}`); the PAT is read from the `ADO_PAT` environment variable.
* NEW: Add `multitool add-reporting-descriptor` verb that appends a SARIF `reportingDescriptor` (from `--input <path>` or stdin) to the staged event log. Default target is `run.tool.driver.notifications[]`; `--rules` retargets to `run.tool.driver.rules[]` and gates the descriptor `id` against `AIRuleIdConvention.IsNovel` (taxonomy-mapped descriptors such as `CWE-89` come from the taxonomy enricher, not this verb). Duplicate `id` within a single event log — against either a prior descriptor event or a descriptor pre-populated on the run-header — is rejected on receipt. Adds `SarifEventKinds.RuleDescriptor` (`rule-descriptor`) and `SarifEventKinds.NotificationDescriptor` (`notification-descriptor`); the replayer merges producer-supplied descriptors into the target list before result-driven auto-registration, so an explicit NOVEL- descriptor pre-empts the minimal one that would otherwise be synthesized from a result's `ruleId`.
* BUG: `SarifUtilities.SarifSchemaUri` now points at the OASIS-final `https://schemastore.azurewebsites.net/schemas/json/sarif-2.1.0.json` instead of the `…sarif-2.1.0-rtm.6.json` prerelease alias. Adds `SarifUtilities.FinalV210SchemaUri`. The rtm.6 URL and `schemastore.org` aliases remain accepted by `SARIF1011.ReferenceFinalSchema`/`ADO1011.ReferenceFinalSchema`/`GH1011` and `PrereleaseCompatibilityTransformer`.
* BUG: `EnumConverter`, `FlagsEnumConverter`, `SarifVersionConverter`, and `DateTimeConverter` produced corrupted output when routed through `JToken.FromObject` or any other `JTokenWriter` target. Text-path output is byte-identical.
...
* BUG: `SARIF2017.ProvideRequiredRegionProperties` no longer fires on physical locations whose region uses `charOffset`/`charLength` or `byteOffset`/`byteLength` form per §3.30 instead of `startLine`. Message text now enumerates the three valid whole-artifact-region shapes. Resolves #2909.
* BUG: `AI1010.ProvideEvidenceBackingUri` and `AI2016.ProvideEvidenceBacking` now accept JSON-array `backing`/`strength` values in `ai/evidence` (in addition to single strings) instead of throwing `InvalidCastException` (surfaced as `ERR998`). AI1010 fires one finding per non-resolving `sarif:` URI in the array. Resolves #2908.
* BUG: `CweGenerateSample.ps1 -GHAzDO` no longer crashes when re-run inside a real ADO pipeline. The deterministic-fixture env override now sets the `SYSTEM_DEFINITIONID` / `SYSTEM_JOBID` / `SYSTEM_JOBNAME` fallback vars alongside their primaries so `AdoPipelineContext.TryDetect`'s "disagrees with" cross-check (BUILD_DEFINITIONID vs SYSTEM_DEFINITIONID) doesn't fire against the agent's injected values.
...
* BRK: Renumber AI validation rules for RFC 2119 compliance (`AI1xxx` = MUST/SHALL error; `AI2xxx` = SHOULD warning/note). `AI2006.ProvideMessageMarkdown` → `AI1005.ProvideMessageMarkdown`, `AI1007.ProvideExploitability` → `AI2014.ProvideExploitability`. The `AI3xxx` series is eliminated.
...
* NEW: Add `AI1003.ProvideRequiredRegionProperties` validation rule — error when result locations lack a `region` or required region properties. Mirrors `SARIF2017.ProvideRequiredRegionProperties` at error level for AI profile.
...
* BUG: Fix incorrect base class in rule `ADO2012.ProvideRequiredReportingDescriptorProperties`.
...
* BUG: Fix NullReference in `SARIF1012.MessageArgumentsMustBeConsistentWithRule` rule validation [#2254]. (<https://github.com/microsoft/sarif-sdk/pull/2254>)
...
* NEW: Validation rule `SARIF2012.ProvideRuleProperties` now checks for the presence of a friendly name in PascalCase in the `name` property, and is renamed from `ProvideHelpUris` to `ProvideRuleProperties`.
...
* NEW: Add validation rule `SARIF1019.RuleIdMustBePresentAndConsistent`, which requires every result to have at least one of `result.ruleId` and `result.rule.id`. If both are present, they must be equal. [#1880](https://github.com/microsoft/sarif-sdk/issues/1880)
* NEW: Add validation rule `SARIF1020.ReferToFinalSchema`, which requires that the $schema property should be present, and must refer to the final version of the SARIF 2.1.0 schema. [#1890](https://github.com/microsoft/sarif-sdk/issues/1890)
...
* BUG: Validation rule `SARIF1018.InvalidUriInOriginalUriBaseIds` was not checking for a trailing slash on `uri` properties in `originalUriBaseIds` if `uriBaseId` was present.
...
* NEW: In validation rule `SARIF1018.InvalidUriInOriginalUriBaseIds`, require `uri` values in `originalUriBaseIds` to end with a slash, per the SARIF spec.
...
* BUG: Analysis rule `SARIF1017.InvalidIndex` incorrectly rejected index-valued properties that referred to taxonomies. [#1678](https://github.com/microsoft/sarif-sdk/issues/1678)
...
* BUG: Validation rule `SARIF1015.UriMustBeAbsolute` incorrectly required `originalUriBaseIds` to be contain URIs. [#1485](https://github.com/microsoft/sarif-sdk/issues/1485)
