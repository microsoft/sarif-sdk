...
* BRK: Remove the `add-notification` verb and the `execution-notification` / `configuration-notification` event kinds; SARIF has no run-level notifications array, so notifications now travel inline on a fully-formed `add-invocation` payload (`toolExecutionNotifications` / `toolConfigurationNotifications`), letting parallel processes each emit their own self-contained invocation.
...
* NEW: Add publishable AI emit-profile input schemas for the `add-invocation` and `add-reporting-descriptor` verbs, plus a novel-rule-descriptor overlay pinning the `--rules` `NOVEL-` id gate, each tied to its C# receipt contract by a drift test.
