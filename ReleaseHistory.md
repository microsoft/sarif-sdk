...
* NEW: The `AI-RULEID-001` rejection envelope now names the canonical fallback for a bare `CWE-<n>` ruleId: when no sharper sub-pattern applies, kebab-case the CWE name as the sub-id (e.g., `CWE-89/sql-injection`). The producer must still author it; the emit chain never fills it in.
