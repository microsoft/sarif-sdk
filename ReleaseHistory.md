...
* NEW: Introduce `Sarif.Mcp.Server` package — a `DotnetTool` (command `sarif-mcp-server`) that will host Model Context Protocol (MCP) operations for producing, merging, and manipulating SARIF files. The 4.7.0 release ships a stub binary to validate the publish pipeline; functionality lands in subsequent releases.
