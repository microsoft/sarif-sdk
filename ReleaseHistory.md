...
* NEW: `sarif emit-finalize` now stamps a `description` on each minted `originalUriBaseIds` entry whose `text` is a SARIF embedded link (§3.11.6) whose anchor (`<repo>@<short-sha>`) links to a root-at-revision URL pinned at the full commit (GitHub `/tree/<sha>`, Azure DevOps `?version=GC<sha>`). A producer-supplied description is preserved.
