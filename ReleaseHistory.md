...
* BUG: `emit-run` no longer reconciles a single `versionControlProvenance` entry against the pipeline when that entry's `repositoryUri` names a different repository than the build enlistment; such an entry (a scan target distinct from the pipeline source repo) is now left untouched, like a multi-entry array, instead of being enriched with — or failing against — the pipeline's `revisionId`/`branch`.
