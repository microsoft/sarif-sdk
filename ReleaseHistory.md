...
* BUG: `emit-finalize` floors a CWE rule descriptor's `name` to its CWE id when the MITRE taxonomy cannot name it — a deprecated category such as `CWE-16` — so the descriptor it injects no longer fails `GHAzDO2012.ProvideRequiredReportingDescriptorProperties`.
