...
* BUG: `emit-finalize` floors a CWE rule descriptor's `name` to its id only when the CWE is a known MITRE Weakness (e.g. under `--no-cwe-enrichment`); a Category, View, or unknown CWE is left nameless rather than normalized.
