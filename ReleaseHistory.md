...
* BUG: `SarifUtilities.SarifSchemaUri` now resolves to the OASIS-final `https://schemastore.azurewebsites.net/schemas/json/sarif-2.1.0.json` instead of the `…sarif-2.1.0-rtm.6.json` prerelease alias. `SarifUtilities.FinalV210SchemaUri` is the new public constant for the URL; `ConvertToSchemaUri(SarifVersion.Current)` returns the same. Existing logs that cite the rtm.6 URL remain accepted by `SARIF1011`/`ADO1011`/`GH1011` (suffix-match against `SchemaVersionAsPublishedToSchemaStoreOrg`), and by `PrereleaseCompatibilityTransformer` (rtm.5/rtm.6 already in the no-op case, joined now by `sarif-2.1.0.json` + `www.schemastore.org` + `json.schemastore.org` aliases). `SARIF2006.UrisShouldBeReachable` skips both URLs symmetrically. Existing `*.sarif` baselines are unchanged. Per-commit content note: the prior SDK-H attempt (#2901) rewrote ~545 baseline files; this revival keeps the changeset minimal because rtm.6 remains legal across all rules.
...
* BUG: `EnumConverter`, `FlagsEnumConverter`, `SarifVersionConverter`, and `DateTimeConverter` corrupted SARIF objects routed through `JToken.FromObject` (or any other `JTokenWriter` target). All four used `JsonWriter.WriteRawValue` with hand-quoted strings; switched to the natural Newtonsoft primitives (`WriteValue` / `WriteStartArray` + `WriteValue` + `WriteEndArray`). Text-path output is byte-identical.
* NEW: Add incremental SARIF authoring foundation in new `Microsoft.CodeAnalysis.Sarif.Emit` namespace. Producers can now author a SARIF log across multiple process invocations via an append-only JSONL event log:
  * `SarifEventLogWriter` / `SarifEventLogReader` — versioned newline-delimited JSON event log (kinds: `run-header`, `result`, `invocation`, `notification`). Writers reject files with a torn trailing line; readers tolerate CRLF, blank lines, and unknown kinds (forward-compatible).
  * `SarifEventReplayer` — replays an event log into a `SarifLog`, auto-registering `tool.driver.rules` from first-sighting `Result.RuleId` (mirroring `SarifLogger.LogRule`) and routing notifications to the last invocation (synthesizing one with `executionSuccessful = true` when none was reported).
  * `AtomicSarifWriter` — writes the destination SARIF via a same-directory temp file + `File.Move(overwrite: true)` so readers see prior-or-new contents, never a partial file. Tolerates consumers that dispose any `StreamWriter` they wrap around the supplied stream.
* NEW: Add `multitool emit-init` and `multitool emit-finalize` verbs that bookend the incremental authoring flow. `emit-init` creates (or refuses to clobber) a `<output>.sarif.wip.jsonl` and seeds a `run-header` event with tool metadata + version-control provenance. `emit-finalize` replays the wip, runs `Microsoft.CodeAnalysis.Sarif.Taxonomies.CweTaxonomyEnricher.Enrich` (unless `--no-cwe-enrichment`), atomically writes the final `<output>.sarif`, and removes the wip (unless `--keep-wip`). External producers append events to the wip between the two verbs.
* NEW: Internal — `Sarif.Multitool.Program.Main` rewritten to use the non-generic `CommandLine.Parser.ParseArguments(args, params Type[])` overload + a `Dispatch(object)` switch. The previous generic `ParseArguments<T1..T16>` cap was at the verb limit and prevented further additions; the new shape removes the ceiling entirely.
...
* BUG: `SARIF2017.ProvideRequiredRegionProperties` no longer fires `Warning_MissingRegionProperty` on physical locations whose region is expressed in a non-line/column form. Per SARIF §3.30, a region may be identified by *one of* line/column (`startLine` [+ optional `startColumn`/`endLine`/`endColumn`]), char-offset (`charOffset` [+ optional `charLength`]), or byte-offset (`byteOffset` [+ optional `byteLength`]). The rule used to fire when `region.startLine == 0` regardless of whether the producer had supplied positional info in one of the other forms — a producer doing the right thing for a whole-artifact finding with `{ byteOffset: 0, byteLength: <size> }` was flagged because `startLine` was 0. The rule now fires only when the region carries *no* positional information in any of the three valid forms, and its message text enumerates the three whole-artifact-region shapes a producer can supply (line, char-offset, or byte-offset form). Resolves #2909.
* BUG: Fix `InvalidCastException` (surfaced as `ERR998.ExceptionInAnalyze`) in `AI1010.ProvideEvidenceBackingUri` and `AI2016.ProvideEvidenceBacking` when a result has `ai/evidence` entries whose `backing` (or `strength`) value is a JSON array of strings rather than a single string. AI security tools that emit evidence with multiple supporting pointers per claim naturally express `backing` as an array, so this was encountered in practice. Both rules now read defensively via a new internal helper `EvidenceJsonReader` that accepts both shapes: a single string yields a one-element list; an array yields its string elements (non-string array elements silently dropped); any other shape yields an empty list. AI1010 now fires one finding per non-resolving `sarif:` URI in the array; AI2016 considers an entry "has backing" when any string-shaped backing value is present. Resolves #2908.
* NEW: Extend `SARIF2004.OptimizeFileSize` to flag explicit emission of the SARIF `-1` "unset index" sentinel (§3.4) on `index`, `ruleIndex`, and `parentIndex` properties (`ArtifactLocation`, `ReportingDescriptorReference`, `ThreadFlowLocation`, `LogicalLocation`, `Address`, `Artifact`, `Result.RuleIndex`). The sentinel value is semantically equivalent to omitting the property; emitting it literally inflates the log without changing meaning. The check distinguishes "the producer wrote `index: -1`" (bloat — fires) from "the producer omitted the property" (the SDK initializes the C# field to `-1` on deserialization but the property isn't physically present in the wire JSON — no fire). Resolves #2910.
