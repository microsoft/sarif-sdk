...
## **v5.0.1** UNRELEASED
* BRK: `multitool emit-init-run` removes the twelve typed content flags shipped in v5.0.0 (`--tool-driver-name`, `--tool-version`, `--tool-driver-semantic-version`, `--information-uri`, `--organization`, `--automation-guid`, `--automation-correlation-guid`, `--ai-origin`, `--vcp-repositoryuri`, `--vcp-revisionid`, `--vcp-branch`, `--srcroot`) and now consumes a SARIF `Run` JSON document via `--input <path>` or stdin, matching the contract of `add-result` / `add-notification` / `add-reporting-descriptor`. `--force-overwrite` is retained.
* BRK: `Microsoft.CodeAnalysis.Sarif.Multitool.AdoPipelineContext.ApplyTo(Run)` is replaced by `bool TryApplyTo(Run run, out string error)`, which stamps `automationDetails.id` plus the four `azuredevops/pipeline/build/*` properties only when absent and returns `false` with a per-field diagnostic when a caller-supplied value conflicts with the env-detected pipeline identity.
* NEW: `multitool emit-init-run` accepts the SARIF `Run` partial-document shape the replayer already supports (`tool`, `language`, `columnKind`, `defaultEncoding`, `defaultSourceLanguage`, `originalUriBaseIds`, `versionControlProvenance`, `automationDetails`, `baselineGuid`, `redactionTokens`, …), unblocking multiple `versionControlProvenance` entries and arbitrary attached `properties` bags the flag surface could not encode. Receipt-time validators reject — without writing the wip — a non-empty-string `tool.driver.name`; non-`https` `tool.driver.informationUri` or `versionControlProvenance[].repositoryUri`; non-`https`/`file` `originalUriBaseIds["SRCROOT"].uri`; non-canonical 8-4-4-4-12 `automationDetails.guid` / `correlationGuid`; `properties["ai/origin"]` outside `{generated, annotated, synthesized}`; a SARIF log document supplied in place of a `Run`; and an ADO field that conflicts with the env-detected pipeline identity. Fields outside the SDK's typed `Run` model are preserved on the `.wip.jsonl` run-header event but, like every other SDK round-trip, are dropped when `emit-finalize` materializes a typed `SarifLog`.
