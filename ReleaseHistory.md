...
* NEW: `sarif merge --merge-runs` now aggregates `run.invocations` across merged runs and rebases each result's `provenance.invocationIndex` accordingly, so result→invocation traceability survives a merge; invocations were previously discarded.
