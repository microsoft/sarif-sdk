...
* NEW: `publish-to-ghas` verb uploads a finalized SARIF to GitHub Advanced Security code scanning (`POST /repos/{owner}/{repo}/code-scanning/sarifs`), deriving `owner/repo`, `commit_sha`, and `ref` from `versionControlProvenance` and reading the bearer token from the env var named by `--token-env-var` (default `GH_TOKEN`); refuses an unpublishable (`--no-repo`) log.
