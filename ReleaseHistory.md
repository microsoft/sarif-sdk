...
* NEW: `publish-to-ghas` verb uploads a finalized SARIF to GitHub code scanning, deriving the target, commit, and ref from `versionControlProvenance` and reading the bearer token from the env var named by `--token-env-var` (default `GH_TOKEN`); refuses an unpublishable (`--no-repo`) log.
