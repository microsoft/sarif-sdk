...
* NEW: Add `CweGenerateSample.ps1` + the deterministic `CweSample.sarif` fixture it produces. The script (now zero-parameter) writes the fully-enriched fixture next to itself; `CweGeneratedSampleTests` hashes before/after as the regression gate. Convention: every taxonomy's `<Tax>GenerateSample.ps1` produces a `<Tax>Sample.sarif` fixture.
