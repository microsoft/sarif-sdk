...
namespace Microsoft.CodeAnalysis.Sarif
{
    ...
    internal sealed class ResultComparer : IComparer<Result>
    {
        ...
        public int Compare(Result left, Result right)
        {
            ...
            // TryReferenceCompares is an autogenerated extension method
            // that will properly handle the case when 'left' is null.
            if (left.Guid.TryReferenceCompares(right.Guid, out compareResult) && compareResult != 0)
            {
                return compareResult;
            }

            compareResult = left.Guid.Value.CompareTo(right.Guid.Value);
            if (compareResult != 0)
            {
                return compareResult;
            }
            ...
            // TryReferenceCompares is an autogenerated extension method
            // that will properly handle the case when 'left' is null.
            if (left.CorrelationGuid.TryReferenceCompares(right.CorrelationGuid, out compareResult) && compareResult != 0)
            {
                return compareResult;
            }

            compareResult = left.CorrelationGuid.Value.CompareTo(right.CorrelationGuid.Value);
            if (compareResult != 0)
            {
                return compareResult;
            }
