            ...
            if (v2Region != null)
            {
                    ...
                    v2Region.CharLength > 0)
                {
                    if (v2Region.StartLine > 0)
                    {
                        ...
                        region.StartLine = v2Region.StartLine ?? 0;
                    ...
                    {
                        region.EndLine = v2Region.EndLine ?? 0;

                        if (v2Region.EndColumn > 0)
                        {
                            region.EndColumn = v2Region.EndColumn ?? 0;
                        }
                        else
                        {
        ...
        private int GetRegionEndColumn(Region v2Region, Uri uri)
        {
            ...
            using (StreamReader reader = GetFileStreamReader(uri, out Encoding encoding))
            {
                if (reader != null)
                {
                    ...
                    if (sourceLine != null)
                    {
                        endColumn = sourceLine.Length >= (v2Region.EndColumn ?? 0)
                            ? v2Region.EndColumn ?? 0
                            : sourceLine.Length + 1;
                    }
                }
