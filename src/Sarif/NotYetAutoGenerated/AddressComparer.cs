...
namespace Microsoft.CodeAnalysis.Sarif
{
    ...
    internal sealed class AddressComparer : IComparer<Address>
    {
        ...
        public int Compare(Address left, Address right)
        {
            ...
            compareResult = Nullable.Compare<BigInteger>(left.RelativeAddress, right.RelativeAddress);
            if (compareResult != 0)
            {
                return compareResult;
            }

            compareResult = Nullable.Compare<BigInteger>(left.Length, right.Length);
            if (compareResult != 0)
            {
                return compareResult;
            }
            ...
            compareResult = Nullable.Compare<BigInteger>(left.OffsetFromParent, right.OffsetFromParent);
            if (compareResult != 0)
            {
                return compareResult;
            }
