...
namespace Microsoft.CodeAnalysis.StaticAnalysisResultsInterchangeFormat
{
    ...
    public static class EnumerableExtensions
    {
        ...
        public static bool RemoveIf<T>(ref T[] array, Func<T, bool> predicate)
        {
            ...
            int newLogicalEnd = RemoveIfImpl(array, predicate);

            if (newLogicalEnd == array.Length)
            {
                return false;
            }
            else
        ...
        /// <summary>Removes elements from a list if they match a predicate.</summary>
        /// <exception cref="ArgumentNullException">Thrown when one or more required arguments are null.</exception>
        /// <typeparam name="T">Type of elements in the list.</typeparam>
        /// <param name="list">The list from which elements shall be removed.</param>
        /// <param name="predicate">The predicate to check against.</param>
        /// <returns>true if any elements were removed from <paramref name="list"/>; otherwise, false.</returns>
        public static bool RemoveIfList<T>(IList<T> list, Func<T, bool> predicate)
        {
            if (list == null)
            {
                return false;
            ...
            int newLogicalEnd = RemoveIfImpl(list, predicate);

            if (newLogicalEnd == list.Count)
            {
                return false;
            }
            else
            {
                for (int idx = list.Count - 1; idx >= newLogicalEnd; --idx)
                {
                    list.RemoveAt(idx);
                }
        ...
        private static int RemoveIfImpl<T, TList>(TList list, Func<T, bool> predicate)
            where TList : class, IList<T>
        {
            ...
            for (int first = 0; first != last; ++first)
            {
                ...
                {
                    list[newLogicalEnd++] = list[first];
                }
            }

            return newLogicalEnd;
        }
