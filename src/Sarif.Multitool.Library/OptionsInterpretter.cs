...
#pragma warning restore IDE0060
        ...
        protected void ConsumeEnvVarsAndInterpretOptions(AnalyzeOptionsBase analyzeOptionsBase)
        {
            ...
            IList<ResultKind> kindsFromEnv = GetOptionListFromEnvVar<ResultKind>(nameof(analyzeOptionsBase.Kind));
            IList<FailureLevel> levelsFromEnv = GetOptionListFromEnvVar<FailureLevel>(nameof(analyzeOptionsBase.Level));
...
#pragma warning restore IDE0060
        ...
        private static IList<T> NullCheckAndDistinctUnionLists<T>(IList<T> firstList, IList<T> secondList)
        {
            if (secondList != null)
            {
                //  Union in C# is distinct
        ...
        private IList<T> GetOptionListFromEnvVar<T>(string propertyName)
        {
            return _environmentVariableGetter
                .GetEnvironmentVariable(FormEnvironmentVariableName(propertyName))?
                .Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries)?
