...
* API BREAKING: Remove 'run.resources' object. Rename 'rule' object to 'messageDescriptor'. Move rule and notification message descriptor objects to 'tool.notificationsMetadata' and 'tool.rulesMetadata'. 'resources.messageStrings' now located at 'tool.globalMessageStrings'. 'rule.configuration' property now named 'messageDescriptor.defaultConfiguration'. 'ruleConfiguration.defaultLevel' and 'ruleConfiguration.defaultRank' simplified to 'ruleConfiguration.level' and 'ruleConfiguration.rank'. Actual rules configuration persisted to new array of ruleConfiguration objects at 'invocation.ruleConfiguration'. https://github.com/oasis-tcs/sarif-spec/issues/311
* API BREAKING: define 'toolComponent' object to persist tool data. The 'tool.driver' component documents the standard driver metadata. 'tool.extensions' is an array of 'toolComponent' instances that describe extensions to the core analyzer. This change also deletes 'tool.sarifLoggerVersion' (from the newly created 'toolComponent' object) due to its lack of utility. Finally, adds 'resultProvenance.extensionIndex' to allow results to be associated with a plug-in. https://github.com/oasis-tcs/sarif-spec/issues/179