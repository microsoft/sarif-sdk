...
{
    /// <summary>
    /// This class keeps track of whether, at the current point in the object model visit, the
    /// index property of the reportingDescriptorReference we are examining (if any) refers to the
    /// rules, notification, or taxa array of the toolComponent to which this reference refers.
    /// This is necessary so that the validator (specifically, rule SARIF1017) can tell if the
    /// index is valid with respect to that array.
    /// </summary>
    internal class ReportingDescriptorReferenceContext : IDisposable
    {
        // Should this be thread-static? Should the whole context be thread-static?
