...
- A worked sample log (generated + synthesized, fully cross-linked, validates clean under
  ...
  convention and gated by a byte-identical regen `[Fact]`. It lives at
  [`docs/ai/samples/grouping-findings/`](samples/grouping-findings/): `GroupingSample.sarif`
  (the fixture), `GroupingGenerateSample.ps1` (the generator), and `src/api/proxy.ts` (the
  embedded source the findings reference). The regen `[Fact]` is
  `GroupingGeneratedSampleTests`.
