...
| AI1012 | ProvideRuleSubId | error | Every `result.ruleId` MUST conform to the AI ruleId convention: a taxonomy sub-ID `CWE-<number>/<sub-id>` (e.g., `CWE-78/api-handler`) or the flat escape hatch `NOVEL-<sub-id>`, with a lowercase kebab-case sub-ID. Rule descriptors (`tool.driver.rules[].id`) use the base ID only (e.g., `CWE-78`). A bare base id is flagged as missing its sub-ID; any other non-conformant value is flagged as malformed. |
