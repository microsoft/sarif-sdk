...
| Shape | Form | Example |
|---|---|---|
| Taxonomy sub-id | `<BASE>/<sub-id>` | `CWE-89/kql-injection-from-config` |
| NOVEL escape hatch | `NOVEL-<sub-id>` | `NOVEL-prompt-injection-via-system-message` |

If an AI tool emits a result whose `ruleId` is anything else — bare `CWE-89`, missing entirely, `cwe-89/foo` (lowercase), `NOVEL-foo/bar` (slash inside a NOVEL- form), etc. — `emit-finalize` exits with a non-zero status and prints an AI-consumable error envelope on stderr listing every offending id at once.
...
1. **The finding maps to a known taxonomy entry.** Use the sub-id form (`<BASE>/<sub-id>`). The replayer registers a descriptor with the *base id* (e.g., `CWE-89`) per [SARIF §3.49.3 (descriptor ids are base-only)](https://docs.oasis-open.org/sarif/sarif/v2.1.0/os/sarif-v2.1.0-os.html#_Toc34317644), keeps the full hierarchical form on `result.ruleId` per [§3.52.4](https://docs.oasis-open.org/sarif/sarif/v2.1.0/os/sarif-v2.1.0-os.html#_Toc34317727), and the CWE taxonomy enricher hydrates the base descriptor with MITRE metadata. The AI gets enriched output for free while staying honest about which sub-pattern it observed.

2. **The finding doesn't fit any taxonomy entry.** Use the NOVEL escape hatch (`NOVEL-<sub-id>`). This keeps non-taxonomy findings emittable without forcing the AI to pretend a CWE applies. NOVEL- is intentionally *flat* — no slash, no hierarchy — to signal "I looked, I did not find a fit, this is new ground". If the AI can connect the finding back to a known taxonomy entry, it MUST use shape #1 instead.
...
```
result.ruleId  ::= taxonomySubId | novelEscape

taxonomySubId  ::= base "/" subId
base           ::= [A-Z][A-Z0-9]* ("-" [A-Za-z0-9]+)+      ; e.g., CWE-89, CVE-2021-12345, OWASP-A01-2021
subId          ::= [^/\s]+                                  ; non-empty, no slashes, no whitespace

novelEscape    ::= "NOVEL-" novelSubId
novelSubId     ::= [A-Za-z0-9]+ ("-" [A-Za-z0-9]+)*         ; one or more dash-separated alphanumeric tokens
```

The NOVEL- form is exclusive: a `ruleId` starting with `NOVEL-` is validated only against the NOVEL grammar. `NOVEL-foo/bar` is rejected — not silently accepted as a sub-id form with `NOVEL-foo` as the base.
...
| `CVE-2021-12345/cve-2021-12345-exploit-via-file-upload` | Sub-id form; CVE base is dash-separated digits |
| `OWASP-A01-2021/broken-access-control-in-tenant-scope` | Sub-id form; OWASP base with year tail |
...
| `cwe-89/foo` | Base must be uppercase |
| `CWE-89/` | Sub-id is empty |
| `CWE-89/a/b` | Sub-id contains a slash |
| `CWE-89/a b` | Sub-id contains whitespace |
| `CWE/foo` | Base does not have a `-` segment (not a recognizable taxonomy id) |
...
Every AI-emitted result.ruleId MUST take one of two shapes:
  1. Taxonomy sub-id  <BASE>/<sub-id>
     e.g., 'CWE-89/kql-injection-from-config'
     Use this whenever the finding maps to a known taxonomy entry.
  ...
  2. NOVEL escape hatch  NOVEL-<sub-id>
     ...
     Use this ONLY when no taxonomy entry fits. The NOVEL- form is
     flat (no slash). If the finding maps to a CWE/CVE/OWASP entry,
     use shape #1 instead.
